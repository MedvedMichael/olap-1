...
export default async function uploadFact({ flight, weather }: { flight: Flight, weather: Weather }, pgService: PgService) {
    const request = `INSERT INTO "FactDelays" (
	    ...
	    "distance") VALUES (
                ...
                (SELECT "dateID" FROM "DimDate" WHERE "month" = $7 AND "day" = $8),
                (SELECT "airlineID" FROM "DimAirline" WHERE "name" = $9),
                (SELECT "timeZoneID" FROM "DimTimeZone" WHERE "name" = $10),
                (SELECT "weatherID" FROM "DimWeather" WHERE "weatherType" = $11 AND "weatherSeverity" = $12),
                $13,
                $14,
                $15,
                $16,
